...
# ساخت فایل پایتون موقت با رنگ‌های تعریف شده
cat > "$WORK_DIR/parser.py" << 'PYEOF'
...
for uri in uris:
    ...
    try:
        ...
        if not raw:
            ...
            print(f"  {C.RED}[✗]{C.NC}  {uri[:70]}")
            print(f"       {C.DIM}{err_msg}{C.NC}")
            continue
        ...
        if 'outbounds' in obj:
            ...
            print(f"  {C.RED}[✗]{C.NC}  {uri[:70]}")
            print(f"       {C.DIM}unexpected JSON shape: {list(obj.keys())}{C.NC}")
            continue
        ...
        remark_str = f"  {C.DIM}# {remark[:35]}{C.NC}" if remark else ''
        print(f"  {C.GREEN}[✓]{C.NC}  {tag:<14}  {host_info:<35}{remark_str}")

    except subprocess.TimeoutExpired:
        errors.append((uri, "xray-knife parse timed out"))
        print(f"  {C.RED}[✗]{C.NC}  {uri[:70]}")
        print(f"       {C.YELLOW}timed out{C.NC}")
    except json.JSONDecodeError as e:
        errors.append((uri, f"JSON parse error: {e}"))
        print(f"  {C.RED}[✗]{C.NC}  {uri[:70]}")
        print(f"       {C.DIM}JSON error: {e}{C.NC}")
    except Exception as e:
        errors.append((uri, str(e)))
        print(f"  {C.RED}[✗]{C.NC}  {uri[:70]}")
        print(f"       {C.DIM}{e}{C.NC}")
...
# اجرای پارسر پایتون
python3 "$WORK_DIR/parser.py" "$INPUT_FILE" "$OUTBOUNDS_JSON"
...
cat > "$WORK_DIR/assembler.py" << 'PYEOF'
import json, sys

outbounds_file = sys.argv[1]
output_file = sys.argv[2]

data = json.load(open(outbounds_file))
...
python3 "$WORK_DIR/assembler.py" "$OUTBOUNDS_JSON" "$OUTPUT_FILE"
