...
BOLD='\033[1m'; NC='\033[0m'; DIM='\033[2m'; WHITE='\033[1;97m'; GREY='\033[0;90m'
...
BANNER
    ...
    echo -e "\033[0;90mAuthor : Soroush Yasini\033[0m"
    echo -e "\033[0;90mRepo   : https://github.com/soroushyasini/proxysmith\033[0m"
    echo -e "\033[0;90mEmail  : soroush.yasini5@gmail.com\033[0m"
    echo -e "\033[0;90mLinkedIn: linkedin.com/in/soroush-yasini\033[0m"
    echo -e "\033[0;90m─────────────────────────────────────────────────────────────\033[0m"
    echo ""
}
print_banner
...
# =============================================================================
# NEW FEATURE: SOURCE SELECTION
# =============================================================================
echo -e "\n${WHITE}📡 Choose config source${NC}"
echo -e "${GREY}──────────────────────────────────────────────────${NC}"
echo " 1) Use default URL (Epodonios All Configs)"
echo " 2) Enter custom subscription URL"
read -rp " Select [1 or 2] (default: 1): " src_choice

if [[ "${src_choice:-1}" == "2" ]]; then
    read -rp " Enter subscription URL: " custom_url
    if [[ -n "$custom_url" ]]; then
        SUB_URL="$custom_url"
        ok "Using custom URL: $SUB_URL"
    else
        warn "No URL entered, falling back to default."
    fi
else
    ok "Using default URL: $SUB_URL"
fi
echo -e "${GREY}──────────────────────────────────────────────────${NC}\n"

# =============================================================================
# NEW FEATURE: CREATE RESULTS FOLDER
# =============================================================================
...
# =============================================================================
# FUNCTIONS TO SAVE CONFIGS AT EACH STAGE
# =============================================================================
save_configs() {
    local stage_name="$1"
    local input_file="$2"
    local output_file="$RESULTS_DIR/${stage_name}.txt"
    
    if [[ -f "$input_file" ]]; then
        # Extract just the URIs (second column) from the tab-separated winners files
        # If the file is from xray-knife parsing, it may already be pure URIs
        if [[ "$input_file" == *"round"*"_winners.txt" ]] || [[ "$input_file" == *"winners.txt" ]] || [[ "$input_file" == *"final_winners.txt" ]]; then
            # It's a winners file (tab-separated: latency\turi)
            awk -F'\t' '{print $2}' "$input_file" > "$output_file"
        elif [[ "$input_file" == *"uris.txt" ]] || [[ "$input_file" == "$WORK_DIR"* ]] && [[ "$input_file" == *".txt" ]] && [[ $(head -1 "$input_file") != *$'\t'* ]]; then
            # It's a plain URI file
            cp "$input_file" "$output_file"
        elif [[ -f "$input_file" ]]; then
            # Fallback: copy as-is
            cp "$input_file" "$output_file"
        fi
        ok "Saved $stage_name → $output_file"
    else
        warn "Could not save $stage_name: $input_file not found"
    fi
}
...
# =============================================================================
# SAVE STEP 1: All fetched configs
# =============================================================================
...
# =============================================================================
# SAVE STEP 3: Configs that passed second test
# =============================================================================
save_configs "02_pass2" "$WORK_DIR/winners.txt"
...
print(f"Round 3: {len(selected)} final configs (from {len(rows)} passing)")
for lat, cfg in selected:
    proto = cfg.split('://')[0]
    print(f"  {lat:>5}ms {proto:<10} {cfg[:65]}")
PYEOF
...
# =============================================================================
# SAVE STEP 4: Configs that passed third test
# =============================================================================
save_configs "03_pass3" "$WORK_DIR/final_winners.txt"

# =============================================================================
# FINAL JSON SAVE: Top 12 configs in V2rayN-friendly format
# =============================================================================
awk -F'\t' '{print $2}' "$WORK_DIR/final_winners.txt" > "$RESULTS_DIR/04_json.txt"
ok "Final 12 configs (V2rayN format) saved to $RESULTS_DIR/04_json.txt"
...
for line in lines:
    ...
    try:
        ...
        obj = json.loads(raw)
        if 'outbounds' in obj:
            ob = next( (o for o in obj['outbounds'] if o.get('protocol') not in ('freedom', 'blackhole')), None )
...
# =============================================================================
# FINAL: Show results folder path
# =============================================================================
echo ""
ok "All done! Results are saved in: $RESULTS_DIR"
